...
export const ConsumerMatchesPage: React.FC = () => {
  ...
  if (!hasRequirement) {
    return (
      <div className="space-y-6 max-w-3xl mx-auto animate-in fade-in duration-300">
        <Card className="bg-amber-500/10 border-amber-500/20 p-6 text-center">
          <AlertCircle size={32} className="mx-auto text-amber-400 mb-3" />
          <h2 className="text-lg font-bold text-white">
            {language === 'hi' ? 'कोई आवश्यकता नहीं मिली' : 'No requirement found'}
          </h2>
          <p className="text-sm text-slate-400 mt-2">
            {language === 'hi'
              ? 'कृपया पहले वॉयस पेज पर अपनी आवश्यकता बताएँ।'
              : 'Please go to the voice page and speak your requirement first.'}
          </p>
          <Button variant="primary" size="md" onClick={() => navigate('/consumer/voice')} className="mt-4">
            {language === 'hi' ? 'वॉयस पर जाएँ' : 'Go to Voice Input'}
          </Button>
        </Card>
      </div>
    );
  ...
  // Invalid intent case (SELLER on buyer page)
  if (!isBuyer) {
    return (
      <div className="space-y-6 max-w-3xl mx-auto animate-in fade-in duration-300">
        <Card className="bg-amber-500/10 border-amber-500/20 p-6 text-center">
          <AlertCircle size={32} className="mx-auto text-amber-400 mb-3" />
          <h2 className="text-lg font-bold text-white">
            {language === 'hi' ? 'SELLER अनुरोध' : 'SELLER request detected'}
          </h2>
          <p className="text-sm text-slate-400 mt-2">
            {language === 'hi'
              ? 'यह SELLER अनुरोध है। किसान मैच केवल BUYER के लिए है। कृपया BUYER के रूप में बोलें।'
              : 'This is a SELLER request. Farmer matching is for BUYER intent only. Please try a BUYER request like “I need 500 kg tomatoes”.'}
          </p>
          <Button variant="outline" size="md" onClick={() => navigate('/consumer/voice')} className="mt-4">
